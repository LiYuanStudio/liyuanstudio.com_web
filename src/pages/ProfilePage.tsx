    ...
    <div className="profile-page">
      ...
      <main className="profile-main">
        ...
        <section className="profile-card" aria-labelledby="blog-settings-title">
          <div className="profile-card-header">
            <h2 id="blog-settings-title">博客设置</h2>
            <span>首页展示 demo</span>
          </div>
          ...
          <form className="profile-form" onSubmit={handleBlogSettingsSubmit}>
            ...
            <select
              id="blog-visible-count"
              value={blogSettings.visibleCount}
              onChange={(event) => {
                setBlogSettingsMessage(null);
            ...
            <input
              ...
              type="text"
              value={blogSettings.featuredSlug}
              onChange={(event) => {
                setBlogSettingsMessage(null);
                setBlogSettings((prev) => ({
                  ...prev,
              ...
              placeholder="留空则按发布时间排序"
            />

            <label className="profile-checkbox-row" htmlFor="blog-show-excerpt">
              <input
                id="blog-show-excerpt"
                type="checkbox"
                checked={blogSettings.showExcerpt}
                onChange={(event) => {
                  setBlogSettingsMessage(null);
                  setBlogSettings((prev) => ({
                    ...prev,
                    showExcerpt: event.target.checked,
                  }));
                }}
              />
              <span>在首页博客卡片显示摘要</span>
            </label>

            {blogSettingsMessage && (
              <p className="profile-success" role="status" data-testid="blog-settings-message">
                {blogSettingsMessage}
              </p>
            )}
